...
<head>
    ...
    <!-- Styles -->
    <link rel="stylesheet" href="{{ asset('modules/events/event_templates/lucky/lib.css') }}">
	<link rel="stylesheet" href="{{ asset('modules/events/event_templates/lucky/template.css') }}">
    ...
    @php
         ...
         $background_theme_url = asset('modules/events/event_templates/lucky/images/background.png');
         if($event->background){
...
<body data-spy="scroll">
                    ...
                    <div class="row">
                        <div class="col-md-6">
							...
							<p class="button-view-more"><a class="page-scroll button transparent" href="#description">View more</a></p>
							                            <div class="d-flex social-content">
                              <!-- Sharingbutton Facebook -->
                              <a class="resp-sharing-button__link"
                                href="https://facebook.com/sharer/sharer.php?u={{ url()->full() }}" target="_blank" rel="noopener"
                              ...
                              <a class="resp-sharing-button__link"
                                href="https://twitter.com/intent/tweet/?text={{ $event->name }}&amp;url={{ url()->full() }}" target="_blank"
                              ...
                              <a class="resp-sharing-button__link" href="mailto:?subject={{ $event->name }}&amp;body={{ url()->full() }}"
                              ...
                              <a class="resp-sharing-button__link"
                                href="https://www.linkedin.com/shareArticle?mini=true&amp;url={{ url()->full() }}&amp;title={{ $event->name }}&amp;summary={{ $event->name }}&amp;source={{ url()->full() }}"
                        ...
                        <div class="col-md-6">
                            <h2 class="h2-heading text-center text-light"><span style="color: #ff9800;">@lang('REGISTER')</span> @lang('FORM')</h2>
                        <!-- Registration Form -->
                            @if(!$eventRegistrationExpired && !$eventExpired && ($event->available_seats == "unlimited" || $event->available_seats > 0))
                                <div class="form-container">
                                    <form id="register-form" action="{{ route('events.public.register', ['slug' => $event->short_slug]) }}"
                                        method="post" class="form-register">
                                        @csrf

                                        <div class="form-group required">
                                            <label class="d-block text-light text-left">@lang('Name')</label>
                                            <input type="text" name="fullname[]" class="form-control form-control-input" required>
                                        </div>

                                        <div class="form-group required">
                                            <label class="d-block text-light text-left">@lang('Email')</label>
                                            <input type="email" name="email[]" class="form-control form-control-input" required>
                                        </div>
                                        
                                        <div class="form-group required">
                                            <label class="d-block text-light text-left">@lang('Birthday')</label>
                                            <input type="date" name="birthday[]" class="form-control form-control-input" required/>
                                        </div>
                                        <div class="form-group required">
                                            <label class="d-block text-light text-left">@lang('Cell Phone')</label>
                                            ...
                                            @for($tmp_index = 0; $tmp_index < $tmp_len; $tmp_index++)
                                                ...
                                                @switch($tmp_items['data_type'][$tmp_index])
                                                    ...
                                                    <div class="form-group @if($tmp_items['is_required'][$tmp_index] == '1') required @endif">
                                                        <label class="d-block text-light text-left">{{ $tmp_items['name'][$tmp_index] }}</label>
                                        ...
                                        @if(count($event->ticket_items) > 0)
                                            <div class="form-group required">
                                                <div class="row">
                                                    <div class="col-md-6">
                                                        <label class="d-block text-light text-left">@lang('Tickets')</label>
                                                    ...
                                                    <div class="col-md-6">
                                                        <label class="d-block text-light text-left">@lang('How many Tickets')</label>
                                        ...
                                        <div class="form-group text-center required">
                                            <label class="text-light">
                                                <input type="checkbox" class="form-check-input" style="height: 26px" required/><a class="text-light" target="_blank" href="{{ url("e/" . $event->name . "/" . $event->short_slug . "/terms-and-conditions") }}">I accept the terms & conditions</a>
                                            </label>
                                        </div>
                                        <div class="form-group">
                                          <label class="d-block text-light text-left">Coupon Code</label>
                                          <input type="text" name="coupon" class="form-control form-control-input" placeholder="Enter here."/>
                                        </div>
                                        <div class="form-group">
    ...
    <div id="subheader">
        <div class="container">
            <div class="row">
                <div class="col-md-12">
                    ...
                    <a class="page-scroll button solid scrolling" href="#header">@lang('REGISTER')</a>
                </div>
            </div> <!-- end of row -->
        </div> <!-- end of container -->
     ...
     <!-- Description -->
     <div id="description" class="cards-2" style="clear: both;">
        <div class="container">
	...
	<script>
        ...
        var slug = "{{ auth()->user() ? (getSlugName(auth()->user()->name) . '/' . (is_null(getTestEvent()) ? "" : getTestEvent()->short_slug)) : "" }}";
        var isFirstLogin = {{ auth()->user() ? auth()->user()->login_nums : 0 }};
        var isLogged = {{ auth()->user() ? 1 : 0 }};
        var eventNums = {{ getEventNums() }};
        var isAdmin = {{ auth()->user() ? (auth()->user()->role == "admin" ? 1 : 0) : 0 }};
    </script>
    <script src="{{ asset('modules/events/event_templates/lucky/lib.js') }}"></script>
	<script src="{{ asset('modules/events/event_templates/lucky/template.js') }}"></script>
    ...
    <script src="{{ asset('vendor/intl-tel-input/build/js/intlTelInput-jquery.min.js') }}"></script>
        ...
        $(document).ready(function() {
            ...
            $("#student_cnt").change(function() {
                ...
                if (available_seats == "unlimited" || Number(available_seats) >= students_nums) {
                    var html = "<h5 class='text-light'>Please add information of the additional attendee.</h5>";
                    for(let i = 1; i < students_nums; i++) {
                        html += `<div class="row mb-2">
                            <div class="col-md-6">
                                <div class="form-group required">
                                    <label class="d-block text-left text-light">Name</label>
                                    <input type="text" name="fullname[]" class="form-control form-control-input" required>
                                </div>
                            </div>
                            ...
                            <div class="col-md-6">
                                <div class="form-group required">
                                    <label class="d-block text-left text-light">Email</label>
                                    <input type="text" name="email[]" class="form-control form-control-input" required>
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="form-group required">
                                    <label class="d-block text-left text-light">Birthday</label>
                                    <input type="date" name="birthday[]" class="form-control form-control-input" required/>
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="form-group required">
                                    <label class="d-block text-left text-light">Cellphone</label>
                                    <input type="tel" name="phone[]" data-inputmask="'mask': '999-999-9999'" class="form-control form-control-input" required/>
                                    <input type="hidden" name="phone_number[]"/>
                                </div>
                            </div>
