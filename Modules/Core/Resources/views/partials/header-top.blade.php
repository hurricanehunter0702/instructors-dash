<nav class="navbar navbar-expand topbar mb-4 static-top">
  ...
  <ul class="navbar-nav">
    ...
    <li class="nav-item {{ (url()->current() == url('/page/' . $page->slug)) ? 'active' : '' }}">
      <a class="nav-link" href="{{ url('/page/' . $page->slug) }}"  target="_blank">{{ $page->title }}</a>
    </li>
    @endforeach
  </ul>
  ...
  <ul class="navbar-nav ml-auto">
    ...
    <li class="nav-item dropdown no-arrow">
      ...
      <div class="dropdown-menu dropdown-menu-right shadow animated--grow-in" aria-labelledby="userDropdown">
        ...
        @endcan
        <a class="dropdown-item account-setting-nav-item" href="{{ route('accountsettings.index') }}">
